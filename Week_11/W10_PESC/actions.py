...
def get_student_info():
    print(">> New student <<")
    student = Student(get_name(), get_section(), get_grade("Spanish grade"), get_grade("English grade"), get_grade("Social grade"), get_grade("Science grade"))
    '''
    ...
    try:
        for index,student in enumerate(students_list):
            if student.full_name == student_to_delete["full_name"] and student.section == student_to_delete["section"]:
                ...
                return [students_list, student_to_delete]
            elif index == len(students_list)-1:
    ...
    if students_list != []:
        try:
            ...
            for student in students_list:
                print(f"{counter}. {student.full_name}")
                counter = counter + 1
...
def calculate_student_average(student_info):
    ...
    try:
        average = ((float(student_info.spanish_grade) + float(student_info.english_grade)) + float(student_info.social_grade) + float(student_info.science_grade)) / 4
    except Exception as ex:
        print(f"Error calculating grade average of {student_info.full_name}: {ex}" )
...
def dispaly_highest_grade_averages(students_list):
    ...
    if students_list != []:
        ...
        for student in [average_1, average_2, average_3]:
            if student != [{},0]:
                print(f"{counter}. Student {student[0].full_name} with average {student[1]}")
                counter = counter + 1
                print(f"> Section: {student[0].section}")
                print("> Grades")
                print(f"   Spanish: {student[0].spanish_grade}")
                print(f"   English: {student[0].english_grade}")
                print(f"   Social: {student[0].social_grade}")
                print(f"   Science: {student[0].science_grade}")
                print("")
    else:
        print("-Any student registered")
        ...
        try:
            ...
            for student in students_list:
                if (float(student.spanish_grade) < 60 or float(student.english_grade) < 60 or float(student.social_grade) < 60 or float(student.science_grade) < 60):
                    print(f"{counter}. Student {student.full_name} from section: {student.section}")
                    counter = counter + 1
        except Exception as ex:
